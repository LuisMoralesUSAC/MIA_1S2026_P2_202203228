...
RegistroRmUsr parsearRegistroRmUsr(const std::string& linea) {
    RegistroRmUsr reg;
...
std::string leerUsersRmUsr(const std::string& ruta_disco, int inicio_particion) {
...
bool escribirUsersRmUsr(const std::string& ruta_disco, int inicio_particion, const std::string& contenido) {
...
std::string comandoRmusr(const std::string& usuario) {
    ...
    if (usuario == "root") {
        return "Error: No se puede eliminar al usuario 'root'";
    }
    ...
    std::string contenido = leerUsersRmUsr(particion.path, particion.start);
    if (contenido.empty()) {
    ...
    while (getline(ss, linea)) {
        ...
        RegistroRmUsr reg = parsearRegistroRmUsr(linea);
        
        if (reg.tipo == 'U' && reg.id != 0 && reg.usuario == usuario) {
            usuario_encontrado = true;
            nuevo_contenido += "0,U," + reg.grupo + "," + reg.usuario + "," + reg.password + "\n";
        } else {
    ...
    if (!escribirUsersRmUsr(particion.path, particion.start, nuevo_contenido)) {
