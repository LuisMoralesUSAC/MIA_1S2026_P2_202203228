...
namespace CommandMkfs {
    ...
    inline std::string execute(const std::string& id, const std::string& type, const std::string& fs = "") {
        ...
        std::string fsParam = toLowerCase(fs);
        int fsType = 2; // EXT2 por defecto
        if (fsParam == "3fs") {
            fsType = 3;
        } else if (fsParam == "2fs" || fsParam.empty()) {
            fsType = 2;
        } else {
        ...
        if (fsType == 3) {
            int journal_bytes = sizeof(Journal);
            bloques_journal = (journal_bytes + 63) / 64;
        }
        ...
        sb.s_free_blocks_count = 3 * n - 2 - bloques_journal;
        ...
        sb.s_block_size = 64;
        sb.s_first_ino = 2;
        sb.s_first_blo = 2 + bloques_journal;
        ...
        // Si EXT3, calcular inicio del journal y escribirlo vacío
        int journal_block_start = sb.s_block_start + (2 * 64); // después de bloques 0 y 1
        if (fsType == 3) {
            sb.s_journal_start = journal_block_start;
            
            Journal journal;
            journal.j_count = 0;
            for (int i = 0; i < 50; i++) {
                memset(journal.j_content[i].i_operation, 0, 10);
                memset(journal.j_content[i].i_path, 0, 32);
                memset(journal.j_content[i].i_content, 0, 64);
                journal.j_content[i].i_date = 0.0f;
            }
            
            file.seekp(journal_block_start, std::ios::beg);
            file.write(reinterpret_cast<const char*>(&journal), sizeof(Journal));
            
            // Re-escribir superblock con journal_start actualizado
            file.seekp(partition.start, std::ios::beg);
            file.write(reinterpret_cast<const char*>(&sb), sizeof(Superblock));
        }
        ...
        for (int i = 0; i < 3 * n; i++) {
            char bit;
            if (i == 0 || i == 1) {
                bit = '1'; // raíz y users.txt
            } else if (fsType == 3 && i >= 2 && i < 2 + bloques_journal) {
                bit = '1'; // bloques reservados para journal
            } else {
                bit = '0';
            }
            file.write(&bit, 1);
        ...
        result << "  Bloques: " << (3 * n) << "\n";
        if (fsType == 3) {
            result << "  Journal: " << bloques_journal << " bloque(s) reservado(s)\n";
            result << "  Journal inicio: " << journal_block_start << "\n";
        }
