...
RegistroUsr parsearRegistroUsr(const std::string& linea) {
    RegistroUsr reg;
...
std::string leerUsersUsr(const std::string& ruta_disco, int inicio_particion) {
...
bool escribirUsersUsr(const std::string& ruta_disco, int inicio_particion, const std::string& contenido) {
...
std::string comandoMkusr(const std::string& usuario, const std::string& password, const std::string& grupo) {
    ...
    std::string contenido = leerUsersUsr(particion.path, particion.start);
    if (contenido.empty()) {
    ...
    while (getline(ss, linea)) {
        ...
        RegistroUsr reg = parsearRegistroUsr(linea);
    ...
    std::string nueva_linea = std::to_string(nuevo_uid) + ",U," + grupo + "," + usuario + "," + password + "\n";
    std::string nuevo_contenido = contenido + nueva_linea;
    
    if (!escribirUsersUsr(particion.path, particion.start, nuevo_contenido)) {
        return "Error: No se pudo actualizar el archivo users.txt";
    }
    
    return "Usuario '" + usuario + "' creado exitosamente con UID: " + std::to_string(nuevo_uid);
}
