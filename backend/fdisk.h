...
namespace CommandFdisk {
    ...
    // Eliminar partición primaria o extendida
    inline std::string deletePrimaryOrExtended(const std::string& path, const std::string& name) {
        std::fstream diskFile(path, std::ios::binary | std::ios::in | std::ios::out);
        if (!diskFile.is_open()) {
            return "Error: No se pudo abrir el disco";
        }
        ...
        // Si es extendida, limpiar todos sus EBRs primero
        if (mbr.mbr_partitions[targetIndex].part_type == 'E') {
            int ebrPos = mbr.mbr_partitions[targetIndex].part_start;
            while (ebrPos != -1) {
                EBR ebr;
                diskFile.seekg(ebrPos, std::ios::beg);
                diskFile.read(reinterpret_cast<char*>(&ebr), sizeof(EBR));
                int nextPos = ebr.part_next;

                // Limpiar EBR con ceros
                EBR ebrVacio;
                diskFile.seekp(ebrPos, std::ios::beg);
                diskFile.write(reinterpret_cast<char*>(&ebrVacio), sizeof(EBR));
        ...
        // Marcar partición como inactiva
        ...
        memset(mbr.mbr_partitions[targetIndex].part_name, 0, 16);
    ...
    inline std::string deleteLogicalPartition(const std::string& path, const std::string& name) {
        std::fstream diskFile(path, std::ios::binary | std::ios::in | std::ios::out);
        if (!diskFile.is_open()) {
            return "Error: No se pudo abrir el disco";
        }
        ...
        while (currentPos != -1) {
            ...
            if (ebr.part_status == '1' && ebrName == name) {
                int nextPos = ebr.part_next;

                // Limpiar este EBR
                EBR ebrVacio;
                ebrVacio.part_next = nextPos;
                diskFile.seekp(currentPos, std::ios::beg);
                diskFile.write(reinterpret_cast<char*>(&ebrVacio), sizeof(EBR));

                // Si había un EBR anterior, apuntarlo al siguiente
                if (prevPos != -1) {
                    EBR ebrPrev;
                    diskFile.seekg(prevPos, std::ios::beg);
                    diskFile.read(reinterpret_cast<char*>(&ebrPrev), sizeof(EBR));
                    ebrPrev.part_next = nextPos;
                    diskFile.seekp(prevPos, std::ios::beg);
                    diskFile.write(reinterpret_cast<char*>(&ebrPrev), sizeof(EBR));
                }
        ...
        diskFile.close();
        return "Error: No se encontró la partición lógica '" + name + "'";
    }

    // Buscar tipo de partición por nombre
    inline char findPartitionType(const std::string& path, const std::string& name) {
        std::ifstream diskFile(path, std::ios::binary);
        if (!diskFile.is_open()) return '\0';
        ...
        for (int i = 0; i < 4; i++) {
            if (mbr.mbr_partitions[i].part_status == '1') {
                ...
                if (partName == name) {
                    diskFile.close();
                    return mbr.mbr_partitions[i].part_type;
                }
    ...
    // Agregar o quitar espacio a una partición
    inline std::string addSpaceToPartition(const std::string& path, int delta, const std::string& name) {
        std::fstream diskFile(path, std::ios::binary | std::ios::in | std::ios::out);
        if (!diskFile.is_open()) {
            return "Error: No se pudo abrir el disco";
        }
        ...
        // Buscar la partición en primarias/extendida
        int targetIndex = -1;
        for (int i = 0; i < 4; i++) {
            if (mbr.mbr_partitions[i].part_status == '1') {
                std::string partName(mbr.mbr_partitions[i].part_name);
        ...
        // Si es partición primaria o extendida
        if (targetIndex != -1) {
            int currentSize = mbr.mbr_partitions[targetIndex].part_size;
            int currentStart = mbr.mbr_partitions[targetIndex].part_start;
            int newSize = currentSize + delta;

            if (newSize <= 0) {
                diskFile.close();
                return "Error: El espacio resultante sería inválido (tamaño <= 0)";
            }

            if (delta > 0) {
                // Verificar que haya espacio libre después de la partición
                int currentEnd = currentStart + currentSize;
                int nextStart = mbr.mbr_size; // Por defecto el fin del disco

                for (int i = 0; i < 4; i++) {
                    if (i == targetIndex) continue;
                    if (mbr.mbr_partitions[i].part_status == '1') {
                        int otherStart = mbr.mbr_partitions[i].part_start;
                        if (otherStart > currentEnd && otherStart < nextStart) {
                            nextStart = otherStart;
                        }
                    }
                }

                int availableSpace = nextStart - currentEnd;
                if (delta > availableSpace) {
                    diskFile.close();
                    return "Error: No hay suficiente espacio libre después de la partición. "
                           "Disponible: " + std::to_string(availableSpace) + " bytes";
                }
            }
            ...
            std::string operacion = (delta > 0) ? "agregaron" : "quitaron";
            return "Se " + operacion + " " + std::to_string(std::abs(delta)) + 
                   " bytes a la partición '" + name + "'\n" +
            ...
            if (mbr.mbr_partitions[i].part_status == '1' &&
                ...
                while (currentPos != -1) {
                    ...
                    if (ebr.part_status == '1' && ebrName == name) {
                        ...
                        if (delta > 0) {
                            // Calcular espacio disponible después de esta lógica
                            int currentEnd = ebr.part_start + currentSize;
                            int nextStart = extEnd;

                            if (ebr.part_next > 0) {
                                EBR ebrNext;
                                diskFile.seekg(ebr.part_next, std::ios::beg);
                                diskFile.read(reinterpret_cast<char*>(&ebrNext), sizeof(EBR));
                                nextStart = ebr.part_next;
                            }
                        ...
                        std::string operacion = (delta > 0) ? "agregaron" : "quitaron";
                        return "Se " + operacion + " " + std::to_string(std::abs(delta)) + 
                               " bytes a la partición lógica '" + name + "'\n" +
                               "  Tamaño anterior: " + std::to_string(currentSize) + " bytes\n" +
                               "  Tamaño nuevo: " + std::to_string(newSize) + " bytes";
                    }
    ...
    inline std::string execute(int size, const std::string& unit, const std::string& path, 
        ...
        try {
            ...
            if (!addStr.empty()) {
                if (name.empty()) {
                    return "Error: -add requiere el parámetro -name";
                }
                ...
                try {
                    ...
                    return "Error: El valor de -add debe ser un número entero";
                }

                int deltaBytes = addValue;
                if (unit == "b" || unit == "B") {
                    deltaBytes = addValue;
                } else if (unit == "k" || unit == "K") {
                    deltaBytes = addValue * 1024;
                } else if (unit == "m" || unit == "M") {
                    deltaBytes = addValue * 1024 * 1024;
                } else {
                    deltaBytes = addValue * 1024;
                }

                return addSpaceToPartition(expandedPath, deltaBytes, name);
            }

            // Si es operación de eliminación
            if (!deleteName.empty()) {
                char tipo = findPartitionType(expandedPath, deleteName);
                if (tipo == '\0') {
                    return "Error: No se encontró la partición '" + deleteName + "'";
                }
                if (tipo == 'L') {
                    return deleteLogicalPartition(expandedPath, deleteName);
                } else {
                    return deletePrimaryOrExtended(expandedPath, deleteName);
                }
            }
