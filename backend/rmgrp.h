...
RegistroUsuarioRm parsearRegistroRm(const std::string& linea) {
    RegistroUsuarioRm reg;
...
std::string leerUsersCompletoRm(const std::string& ruta_disco, int inicio_particion) {
...
bool escribirUsersCompletoRm(const std::string& ruta_disco, int inicio_particion, const std::string& contenido) {
...
std::string comandoRmgrp(const std::string& nombre_grupo) {
    ...
    std::string contenido = leerUsersCompletoRm(particion.path, particion.start);
    if (contenido.empty()) {
    ...
    while (getline(ss, linea)) {
        ...
        RegistroUsuarioRm reg = parsearRegistroRm(linea);
        
        if (reg.tipo == 'G' && reg.id != 0 && reg.grupo == nombre_grupo) {
            grupo_encontrado = true;
            nuevo_contenido += "0,G," + reg.grupo + "\n";
        } else {
    ...
    if (!grupo_encontrado) {
        return "Error: El grupo '" + nombre_grupo + "' no existe";
    }

    if (!escribirUsersCompletoRm(particion.path, particion.start, nuevo_contenido)) {
        return "Error: No se pudo actualizar el archivo users.txt";
    }
    
    return "Grupo '" + nombre_grupo + "' eliminado exitosamente";
}
